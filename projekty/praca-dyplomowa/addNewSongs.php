<?php
require 'includes/handlers/addNewSongsHandler.php';
    ...
    <div class="row h-100">
        ...
        <div class="col-10 h-100 sticky-top overflow-auto" style="background-color: #343434">
            <div class="container-fluid">
                <form action="addNewSongs.php" method="post" enctype="multipart/form-data">
                    <h1>Add new songs to database</h1>
                    <div class="row md-3">
                        <div class="col-2">
                            <label for="numberOfForms" class="form-label">Number of songs in playlist</label>
                            <input type="number" id="numberOfForms" name="numberOfForms" class="form-control" value=<?php rememberNumberOfForms(); ?> >
                            <button type="submit" name="generateSongsFormButton" class="btn btn-primary">Generate</button>
                        </div>
                    </div>
                    <?php if (isset($music)) $music->returnAllMessages(); ?>
                    <?php Music::renderForm($dbh);?>
                </form>
            </div>
        </div>
    </div>
